  ...
  $.FE.PLUGINS.oembed = function(editor) {
    ...
    const initInsertPopup = function() {
      ...
      const buttons = `<div class="fr-buttons">
                ${editor.button.buildList(editor.opts.oembedInsertButtons)}
            </div>`;
    ...
    const initEditPopup = function() {
      const buttons = `<div class="fr-buttons">
                ${editor.button.buildList(editor.opts.oembedEditButtons)}
            </div>`;
    ...
    const addNewOembed = function(src) {
      ...
      $oembed.toggleClass('fr-draggable', editor.opts.oembedMove);

      editor.events.focus(true);
      editor.selection.restore();
      editor.undo.saveStep();

      if (editor.opts.oembedSplitHTML) {
          ...
          editor.$el.find('.fr-oembed')
            ...
            .each(function() {
              ...
              try {
                ...
                editor.opts.oembedEmbedFactory(src).then(embed => $this.html(embed));
              } catch (e) {
